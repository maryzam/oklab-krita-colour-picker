...
| 1 | Pending | Honest model contract | `SelectorModel` ABC + defaults; split selector-model implementations under `oklab_colour_picker/models/` behind the stable `selector_models.py` facade; delete all `getattr` probes and disk `_snapped_colour_at` override; unify disk geometry helper. | No behaviour change; model tests + import-discipline green; model package covered by pure/lower-layer guardrails |
